...
with st.sidebar:
    ...
    num_results = st.number_input("Number of results to display", min_value=3, max_value=50, value=5, step=1)
...
query = st.text_input("Your question here...", placeholder="e.g., Top 10 3-bedroom houses under $2000")
use_ai = st.checkbox("Use AI-powered answers", value=True)

if st.button("Ask Genie"):
    index, meta = load_index_and_meta()
    if index is None or meta is None:
        st.error("Property index not built. Click 'Build Property Index' in the sidebar.")
    elif not query:
        ...
        with st.spinner("🧞‍♂️ Searching properties..."):
            ...
            # Override num_results if query has 'Top N'
            ...
            if analytic_ans:
                ...
                if final_df.empty:
                    st.warning("No properties match your search and filter criteria.")
                else:
                    st.subheader("💬 Genie's Summary")
                    answer = synthesize_answer_with_context(query, final_df, use_openai=use_ai, top_n=display_num)
                    st.markdown(answer)

                    st.subheader(f"🏡 Top {display_num} Relevant Properties")
